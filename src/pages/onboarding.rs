        ...
        <div class="flex min-h-screen bg-cream">
            ...
            <div class="flex flex-col justify-center items-center py-12 px-6 w-full sm:px-12 lg:w-1/2 xl:w-2/5">
                <div class="w-full max-w-lg">
                        ...
                        view! {
                            <div>
                                ...
                                <div class="grid grid-cols-2 gap-3 mb-8">
                                    <button
                                        class="p-4 text-left rounded-xl border transition-all duration-200 cursor-pointer border-stone-200/60 bg-surface dark:border-stone-700 dark:hover:border-primary-light/40 hover:border-primary/40 hover:bg-primary/5"
                                        on:click=move |_| apply_template("indoor_shelves")
                                    >
                                    ...
                                    <button
                                        class="p-4 text-left rounded-xl border transition-all duration-200 cursor-pointer border-stone-200/60 bg-surface dark:border-stone-700 dark:hover:border-primary-light/40 hover:border-primary/40 hover:bg-primary/5"
                                        on:click=move |_| apply_template("windowsill")
                                    >
                                        <div class="mb-2 text-2xl">"🪟"</div>
                                    ...
                                    <button
                                        class="p-4 text-left rounded-xl border transition-all duration-200 cursor-pointer border-stone-200/60 bg-surface dark:border-stone-700 dark:hover:border-primary-light/40 hover:border-primary/40 hover:bg-primary/5"
                                        on:click=move |_| apply_template("greenhouse")
                                    >
                                    ...
                                    <button
                                        class="p-4 text-left rounded-xl border transition-all duration-200 cursor-pointer border-stone-200/60 bg-surface dark:border-stone-700 dark:hover:border-primary-light/40 hover:border-primary/40 hover:bg-primary/5"
                                        on:click=move |_| apply_template("outdoor")
                                    >
                                        <div class="mb-2 text-2xl">"☀️"</div>
                    ...
                    {move || (step.get() == 2).then(|| {
                        let current_zones = zones.get();
                        let indoor: Vec<&PendingZone> = current_zones.iter().filter(|z| z.location_type == "Indoor").collect();
                        let outdoor: Vec<&PendingZone> = current_zones.iter().filter(|z| z.location_type == "Outdoor").collect();
                        ...
                        view! {
                            <div>
                                ...
                                {if !indoor.is_empty() {
                                    Some(view! {
                                        <div class="mb-6">
                                            <h3 class="mb-3 text-xs font-semibold tracking-widest uppercase text-stone-400">"Indoor Zones"</h3>
                                            <div class="flex flex-col gap-2">
                                                {indoor.iter().map(|zone| {
                                                    let border = border_accent(&zone.light_level);
                                                    let light_class = light_badge_class(&zone.light_level);
                                                    view! {
                                ...
                                {if !outdoor.is_empty() {
                                    Some(view! {
                                        <div class="mb-6">
                                            <h3 class="mb-3 text-xs font-semibold tracking-widest uppercase text-stone-400">"Outdoor Zones"</h3>
                                            <div class="flex flex-col gap-2">
                                                {outdoor.iter().map(|zone| {
                                                    let border = border_accent(&zone.light_level);
                                                    let light_class = light_badge_class(&zone.light_level);
                                                    view! {
                                                        <div class=format!("p-4 rounded-xl border-l-4 bg-surface border border-stone-200/60 dark:border-stone-700 {}", border)>
