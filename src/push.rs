...
pub async fn send_push(
    ...
    .map_err(|e| crate::error::AppError::Network(format!("VAPID key error: {}", e)))?;

    let sig = sig_builder
        .build()
        .map_err(|e| crate::error::AppError::Network(format!("VAPID sign error: {}", e)))?;

    builder.set_vapid_signature(sig);

    let message = builder
        .build()
        .map_err(|e| crate::error::AppError::Network(format!("Push message build error: {}", e)))?;

    let client = IsahcWebPushClient::new()
        .map_err(|e| crate::error::AppError::Network(format!("Push client error: {}", e)))?;

    client
        .send(message)
        .await
        .map_err(|e| crate::error::AppError::Network(format!("Push send error: {}", e)))?;
